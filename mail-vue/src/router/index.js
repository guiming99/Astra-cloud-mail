...
const routes = [
    {
        path: '/', name: 'layout', redirect: '/inbox', component: () => import('@/layout/index.vue'), children: [
            ...
            { path: '/contacts', name: 'contact', component: () => import('@/layout/contact/index.vue'), meta: { title: 'contacts', name: 'contact', menu: true } },
...
function loadBackground(next) {
    ...
    if (settingStore.settings.background) {
        const src = cvtR2Url(settingStore.settings.background); const img = new Image(); img.src = src;
        img.onload = () => next(); img.onerror = () => next(); setTimeout(() => next(), 3000)
    } else next()
}
router.afterEach((to) => {
    clearTimeout(timer); if (first) removeLoading(); else NProgress.done();
    const uiStore = useUiStore(); if (to.meta.menu) uiStore.accountShow = ['content', 'email', 'send'].includes(to.meta.name) && window.innerWidth > 767;
    if (window.innerWidth < 1025) uiStore.asideShow = false; first = false
})
