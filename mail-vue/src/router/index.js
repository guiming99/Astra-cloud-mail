...
const routes = [
    {
        ...
        children: [
            {
                path: '/inbox', name: 'email', component: () => import('@/views/email/index.vue'),
                meta: { title: 'inbox', name: 'email', menu: true }
            },
            {
                path: '/mail', name: 'content', component: () => import('@/views/content/index.vue'),
                meta: { title: 'message', name: 'content', menu: false }
            },
            {
                path: '/contacts', name: 'contact', component: () => import('@/layout/contact/index.vue'),
                meta: { title: 'contacts', name: 'contact', menu: true }
            },
            {
                path: '/settings', name: 'setting', component: () => import('@/views/setting/index.vue'),
                meta: { title: 'settings', name: 'setting', menu: true }
            },
            {
                path: '/starred', name: 'star', component: () => import('@/views/star/index.vue'),
...
router.beforeEach((to, from, next) => {
    ...
    if (!token && to.name === 'login') { loadBackground(next); return }
...
function loadBackground(next) {
    ...
    if (settingStore.settings.background) {
        ...
        img.onload = () => { next() };
        img.onerror = () => { console.warn("背景图片加载失败:", img.src); next() };
        setTimeout(() => { console.warn("背景加载超时，已放行"); next() }, 3000)
    } else next()
}
router.afterEach((to) => {
    clearTimeout(timer)
    ...
    if (to.meta.menu) {
        if (['content', 'email', 'send'].includes(to.meta.name)) uiStore.accountShow = window.innerWidth > 767;
        else uiStore.accountShow = false
    }
    if (window.innerWidth < 1025) uiStore.asideShow = false
    first = false
})
function removeLoading() { const doc = document.getElementById('loading-first'); if (!doc) return; doc.remove() }
